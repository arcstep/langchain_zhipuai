...
class BaseChatZhipuAI(BaseChatModel, ABC):
    ...
    @abstractmethod
    async def _sse_ainvoke(prompt: Any, stop: Optional[List[str]] = None, **kwargs):
        """异步的SSE调用"""
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        response = self._invoke(prompt, stop, **kwargs)
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        for response in _sse_invoke(prompt, stop, **kwargs):                
            if not isinstance(response, dict):
                response = response.dict()
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        default_chunk_class = AIMessageChunk
        async for response in _sse_ainvoke(prompt, stop, **kwargs):
            if not isinstance(response, dict):
                response = response.dict()
            if len(response["choices"]) == 0:
                continue
...
class ChatZhipuAI(BaseChatZhipuAI):
    ...
    async def _sse_ainvoke(prompt: Any, stop: Optional[List[str]] = None, **kwargs):
        ...
        return await loop.run_in_executor(None, create_completions)
